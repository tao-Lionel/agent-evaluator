...
前三个维度是"确定性"的——有标准答案可以对照。但有些场景没有标准答案，比如"帮我推荐一个适合送女朋友的礼物"。这时候需要用另一个 LLM 来当评委。
...
这借鉴了 τ-bench（Yao et al., ICLR 2025）的设计。它的实验发现：即使是 GPT-4o 这样的顶级模型，在零售场景的 pass^8 也不到 25%——Agent 的失败往往是"某一个维度彻底崩溃"，而不是"每个维度都差一点"。乘积评分能精确捕捉这种模式。

> 注：Anthropic 已将 τ-bench 作为 Claude 系列模型的核心评估基准之一，Claude 3.5 Sonnet / 3.7 / 4 的发布都使用 τ-bench 展示 Agent 能力进展。
